...
用法：
    ...
    python skills/mock/cli.py --help          # 打印本说明

示例：
    # 无参方法
    python skills/mock/cli.py market.get_summary
    python skills/mock/cli.py market.get_intraday_analysis
    python skills/mock/cli.py trading.get_positions
    python skills/mock/cli.py report.submit_summary watch "盯盘总结文本"

    # 位置参数（按方法签名类型注解自动转换：str/int/float）
    python skills/mock/cli.py market.get_stock_info 603019
    python skills/mock/cli.py market.get_strategy_trend_stocks 6
    python skills/mock/cli.py trading.buy 603019 45.0 100

    # 具名参数（覆盖默认值；支持 --key value 与 --key=value 两种写法）
    python skills/mock/cli.py market.get_strategy_trend_stocks 6 --limit 50
    python skills/mock/cli.py market.get_daily_k_data 603019 --days 10
    python skills/mock/cli.py report.submit_summary review "复盘总结文本" --date 2026-07-06
    python skills/mock/cli.py trading.buy --stock_code 603019 --price 45.0 --volume 100

约定：
    - 工作区根 = 项目根目录（脚本所在的最外层目录）；本文件位于 skills/mock/cli.py，启动时自动把项目根目录注入 sys.path
...
# 把项目根目录注入 sys.path 最前，使 `from skills.mock import ...` 无论 cwd 在哪都能解析。
# __file__ = <项目根>/skills/mock/cli.py → parents[2] = 项目根目录
_PROJECT_ROOT = str(Path(__file__).resolve().parents[2])
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from skills.mock import market, report, trading  # noqa: E402

# 模块白名单：只允许调用这三个接口调用层模块的方法，防止任意 import 注入
_MODULES = {"market": market, "trading": trading, "report": report}

# 方法名合法字符（防 method 含点号 / 斜杠等做路径穿越）
_METHOD_NAME_RE = re.compile(r"^[a-zA-Z_][a-zA-Z0-9_]*$")

# 用法说明（--help 时打印）
_USAGE = """柚子 AI SDK 通用方法调用器
...
示例:
    ...
    python skills/mock/cli.py market.get_daily_k_data 603019 --days 10
    python skills/mock/cli.py trading.get_positions
    python skills/mock/cli.py trading.get_account
    python skills/mock/cli.py trading.buy --stock_code 603019 --price 45.0 --volume 100
    python skills/mock/cli.py report.submit_summary watch "盯盘总结文本"

类型转换: 按方法签名类型注解自动转换（str/int/float/bool/dict/list）。
输出: 方法返回值 JSON（保留信封契约）；异常输出结构化错误 JSON 且退出码非零。
"""
...
def _emit_error(code: int, message: str, target: str = "", **extra) -> int:
    ...
    入参：
        ...
        target: str（可选）- 出错的 module.method
...
# 列出全部可用方法（module.method 每行一个），供 --list 发现
def _print_method_list() -> None:
    """列出三个 SDK 模块的全部公开方法（每行一个 module.method）。"""
...
def main(argv: list[str]) -> int:
    """主入口：解析命令行 → 定位 SDK 方法 → 按签名转换参数 → 调用 → JSON 输出。
    ...
    # 解析目标方法名：必须是 module.method 两段式
    if "." not in target:
        return _emit_error(400, f"目标方法格式应为 <module>.<method>，但传入 {target!r}", target)
    mod_name, _, method_name = target.partition(".")
    if mod_name not in _MODULES:
        return _emit_error(
            404,
            f"模块 {mod_name!r} 不在白名单（仅允许 {'/'.join(_MODULES)}）",
            target,
        )
    if not _METHOD_NAME_RE.match(method_name):
        return _emit_error(400, f"方法名 {method_name!r} 含非法字符", target)

    module = _MODULES[mod_name]
    method = getattr(module, method_name, None)
    if method is None or not callable(method):
        return _emit_error(404, f"模块 {mod_name} 下不存在方法 {method_name!r}", target)

    # 解析命令行参数
    try:
        positionals, kwargs = _parse_tokens(tokens)
    except _ArgError as exc:
        return _emit_error(400, str(exc), target)
    ...
    try:
        ...
        if len(positionals) > len(param_names):
            return _emit_error(
                400,
                f"位置参数过多：传入了 {len(positionals)} 个，但方法 {target} 最多接收 {len(param_names)} 个",
                target,
        ...
        if unknown:
            return _emit_error(
                400,
                f"方法 {target} 不存在参数 {unknown}（合法参数: {param_names}）",
                target,
            )
        ...
        return _emit_error(400, f"参数绑定失败: {exc}", target)
