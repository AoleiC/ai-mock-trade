...
|---|---|---|---|
| **冰点**：今天本身就是第 1 个冰点日 | —（收盘才确认） | 不买 | 单冰日不操作，只记录观察——冰点要收盘定格、次日生效，当天仍按前一阶段（多为退潮）的规矩管 |
| **冰点**：昨天刚出现第 1 个冰点日 | 今天回暖了 | 10:00 以后 | 小仓试错，买当天资金一起进攻的板块里还没大涨的股；午后只做低吸 |
| **冰点**：昨天刚出现第 1 个冰点日 | 今天还在冰 | 尾盘 14:30~14:55 | 可以考虑低吸，只买近 10 天至少 3 次进市场前三、且当天已跌出涨停前三的主线板块——冰点里还逆势坚挺的可能补跌，不碰；别抄底 |
| **冰点**：已连冰 2 天以后的日子 | 冰着或回暖都行 | 全天 | 小仓试错；回暖了就买当天资金共振的方向，可以放宽一点仓位；还冰就只买已回调的主线板块（逆势坚挺的不碰）；没有符合条件的就不出手 |
| **冰点反弹延续**：冰点簇还没终结，昨天已脱离冰点（回暖的第 2 天） | 继续回暖 | 全天 | 当冰点博弈的延续对待：当帧回暖 + 当天资金共振的方向小仓试错（仓位不放宽）；当天卖出后可以再买（重新核齐全部条件）；还在冰就不适用；再往后一天（回暖第 3 天）回归退潮期不买 |
...
| **退潮**：情绪走弱（冰点反弹延续日除外，见上表） | — | 不买 | **没有例外——节点不对，题材再强都不买** |

市场节点一票否决：节点不对，题材再强都不买。节点对了，买入只管三件事：

- **时间点**：按上表窗口执行；板块级机会——某板块连续 2 天市场最热（涨停数居前三）时，升温、冰点、冰点反弹延续节点的尾盘 14:30~14:55 可参与（不追涨停，当天错过不追认）
...
**禁开仓**：退潮期节点（冰点反弹延续日除外）、高潮期第 2 天起、当日情绪保守且资金退潮（冰点先手通道 A/A2/B/D 豁免——冰点期此属常态而非恶化）、交易所监管名单等情形下，禁止一切新开仓。
